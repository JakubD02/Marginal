...
from models import Scenario, FixedCost, Product, Ingredient, RecipeItem
...
with get_session() as session:
    ...
    base = Ingredient(name="Vanilla base", purchase_price=Decimal("15"), purchase_unit=Unit.KG, unit_size=1.0)
...
with get_session() as session:
    ...
    for product in scenario.products:
        for item in product.recipe_items:
            print(f"  {product.name}: {item.quantity}{item.unit} {item.ingredient.name}")